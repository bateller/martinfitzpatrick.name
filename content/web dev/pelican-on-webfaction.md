...
> If you haven't set up github with your SSH keys you'll need to do that now. Generate a key on your Webfaction host using `ssh-keygen`. Then at the SSH prompt enter `less ~/.ssh/id_rsa.pub` to see the generated key and copy-paste it into your [Github SSH settings](https://github.com/settings/ssh). Hit `q` to exit less.
